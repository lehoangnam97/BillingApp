...
namespace InHoaDonBaCang
{
    public partial class lbHoaDon : Form
    {
        ...
        private string filePath = AppDomain.CurrentDomain.BaseDirectory + "\\LuuHoaDon\\";
        ...
        private void btnIn_Clicked(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    string fileName = filePath + "MS" + ReplaceToStandard(txtSoHoaDon.Text) + "_" +
                                      ReplaceToStandard(txtNgayHoaDon.Text);


                    if (File.Exists(fileName + ".pdf"))
                    {
                        int indexFileTail = 1;
                        string fileTail = " (" + indexFileTail + ")";
                        while (File.Exists(fileName + fileTail + ".pdf"))
                        {
                            fileTail = " (" + indexFileTail++ + ")";
                        }
                        fileName += fileTail;
                    }
                    ...
                    {
                        MessageBox.Show("Không kết nối với máy in nào !");
                        return;
                    }

                    if (File.Exists("printer.txt"))
                    {
                        _PrintDocument.PrinterSettings.PrinterName = readPrinterName("printer.txt");
                    }
                    else
                    {
                        File.WriteAllText("printer.txt", "XPrinter");
                    }
                    _PrintDocument.PrinterSettings.PrinterName = readPrinterName("printer.txt");
                    _PrintDocument.PrinterSettings.PrintFileName = fileName;
                    _PrintDialog.Document = _PrintDocument; //add the document to the dialog box


                    //MessageBox.Show(readPrinterName("printer.txt"));
        ...
        private void _CreateReceipt(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            {
                string strStt = "";
                string strTenHang = ""; //string strDvt = "";
                string strSoLuong = "";
                string strDonGia = "";
        ...
        private string ReplaceToStandard(string text)
        {
            if (text.Contains('/'))
                text = text.Replace('/', '-');
            if (text.Contains('/'))
        ...
        private void btnHienNoiLuu_Click(object sender, EventArgs e)
        {
            if (!Directory.Exists(filePath))
            {
                Directory.CreateDirectory(filePath);
        ...
        private string readPrinterName(string filePath)
        {
            string printerName = "";
            printerName = System.IO.File.ReadAllText(filePath);
            return printerName;
        }
